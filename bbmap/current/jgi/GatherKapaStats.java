...
//		outstream.println("Invalid Lines:     \t"+(linesProcessed-linesOut));
	...
	private void printResults(ByteStreamWriter bsw){
		...
		for(TagData td : list){
			...
			keys.addAll(td.map.keySet());
			Collections.sort(keys);
			final int len=td.timesSeen;
			for(String key : keys){
				double[] ppmk=td.getArray(key);
//				if(ppmk.length<seen){
...
//                    ],
	...
	class TagData implements Comparable<TagData> {
		...
		void add(String tag, double ppmk){
			ArrayList<Double> list=map.get(tag);
			if(list==null){
...
//			double[] array=new double[list.size()];
		...
		HashMap<String, ArrayList<Double>> map=new HashMap<String, ArrayList<Double>>(203);
