...
//		ByteStreamWriter tsw=new ByteStreamWriter((out==null ? "stdout" : out) , overwrite, append, false);
		...
		if(FORMAT<1){
			...
			jo.add("gap_pct", String.format("%.5f", (scaflen-contiglen)*100f/scaflen));
			...
			jo.add("scaf_logsum", sLogSumString);
			jo.add("scaf_powsum", sPowSumString);
			jo.add("ctg_logsum", cLogSumString);
			jo.add("ctg_powsum", cPowSumString);
			jo.add("asm_score", sAssemblyScoreString);
			...
			jo.add("gc_avg", String.format("%.5f", (counts[1]+counts[2])*1.0/(counts[0]+counts[1]+counts[2]+counts[3])));
			jo.add("gc_std", String.format("%.5f", gc_std));
